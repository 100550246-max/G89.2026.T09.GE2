...
# --- METHOD 1 ---
    ...
    def register_project(self,
        ...
        # --- 1. Import and instantiate the class ---
        from uc3m_consulting.enterprise_project import EnterpriseProject

        # Pass the validated data to the constructor
        new_project = EnterpriseProject(company_cif, project_acronym, operation_name, department, date, budget)
        ...
        # The manual requires raising an exception if a project with the same name and CIF already exists
        for p in projects_list:
            if p.get("company_cif") == company_cif and p.get("project_description") == operation_name:
    ...
    def register_document(self, input_file: str):
        ...
        def check_duplicates(ordered_pairs):
            ...
            for k, v in ordered_pairs:
                if k in d:
                    raise KeyError(f"Duplicate key detected: {k}")
                d[k] = v
            return d
        ...
        if type(filename) is not str or not re.fullmatch(r"^[a-zA-Z0-9]{8}\.(pdf|docx|xlsx)$", filename):
